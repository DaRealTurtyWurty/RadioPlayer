...
public final class FfmpegAudioStream implements AudioStream {
    ...
    public static AudioStream tryCreate(String url, int sampleRate) throws IOException {
        String configuredPath = MediaPlayer.config().ffmpegExecutablePath;
        if (configuredPath == null || configuredPath.isBlank()) {
            MediaPlayer.LOGGER.warn("Lavaplayer cannot play {}; no FFmpeg executable is configured", url);
            return null;
        }
        ...
        try {
            ...
            MediaPlayer.LOGGER.warn("Configured FFmpeg path is invalid: {}", configuredPath, exception);
            return null;
        }

        if (!Files.isRegularFile(executable)) {
            MediaPlayer.LOGGER.warn("Configured FFmpeg executable does not exist: {}", executable);
            return null;
        }

        MediaPlayer.LOGGER.info("Lavaplayer cannot play {}; trying configured FFmpeg at {}", url, executable);
    ...
    public void close() {
        try {
            ...
            MediaPlayer.LOGGER.debug("Could not close FFmpeg audio stream", exception);
        }
