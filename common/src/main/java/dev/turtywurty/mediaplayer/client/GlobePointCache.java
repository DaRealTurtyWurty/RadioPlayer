...
public final class GlobePointCache {
    ...
    private static final ExecutorService POINT_LOADER_EXECUTOR = Executors.newSingleThreadExecutor(task -> {
        var thread = new Thread(task, MediaPlayer.MOD_ID + "-globe-point-loader");
        ...
        synchronized (LOCK) {
            ...
            CompletableFuture<List<GlobePoint>> future = CompletableFuture
                    ...
                    .whenComplete((_, throwable) -> {
                        if (throwable != null)
                            MediaPlayer.LOGGER.error("Failed to load globe points for cell {}", cellKey, throwable);
        ...
        synchronized (LOCK) {
            ...
            pointLoadFuture = CompletableFuture
                    ...
                    .whenComplete((ignored, throwable) -> {
                        if (throwable != null) {
                            MediaPlayer.LOGGER.error("Failed to load globe points", throwable);
                            synchronized (LOCK) {
    ...
    private static String createUserAgent() {
        return MediaPlayer.MOD_ID + "/" + MediaPlayer.VERSION + " (Minecraft " + MediaPlayer.MINECRAFT_VERSION + "; " + System.getProperty("os.name").toLowerCase(Locale.ROOT) + ")";
    }
    ...
    private static List<Station> fetchStations(int offset, int limit) {
        ...
        for (String endpoint : RADIO_BROWSER_ENDPOINTS) {
            try {
                ...
                MediaPlayer.LOGGER.info("Loading {} globe stations from RadioBrowser endpoint: {} offset {}", limit, endpoint, offset);
                return browser.listStationsWithAdvancedSearch(Paging.at(offset, limit), STATION_SEARCH);
            } catch (RuntimeException exception) {
                lastException = exception;
                MediaPlayer.LOGGER.debug("Failed to load globe points from RadioBrowser endpoint: {}", endpoint, exception);
            }
        }
    ...
    private static List<Station> fetchStationsInCell(CellBounds bounds, int offset, int limit) {
        ...
        for (String endpoint : RADIO_BROWSER_ENDPOINTS) {
            try {
                ...
                MediaPlayer.LOGGER.info("Loading {} globe stations from RadioBrowser endpoint: {} offset {} cell {}",
                ...
                lastException = exception;
                MediaPlayer.LOGGER.debug("Failed to load globe cell points from RadioBrowser endpoint: {}", endpoint, exception);
            }
        }
